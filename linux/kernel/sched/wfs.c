...
static struct task_struct *pick_task_wfs(struct rq *rq)
{
    ...
    printk(KERN_DEBUG "WFS: PICKED task PID %d, VFT=%llu, weight=%u, %u tasks in queue\n",
           next_task->pid, wfs_se->vft, wfs_se->weight, wfs_rq->wfs_nr_running);
...
static void set_next_task_wfs(struct rq *rq, struct task_struct *p, bool first)
{
    ...
    printk(KERN_DEBUG "WFS: SET_NEXT task PID %d on CPU %d (first=%d), %u tasks in queue\n", 
           p->pid, cpu, first, wfs_rq->wfs_nr_running);
}
...
static void switched_to_wfs(struct rq *rq, struct task_struct *p)
{
    ...
    printk(KERN_INFO "WFS: Task PID %d SWITCHED_TO WFS class on CPU %d, %u tasks in queue\n", 
           p->pid, cpu, wfs_rq->wfs_nr_running);
...
static void switched_from_wfs(struct rq *rq, struct task_struct *p)
{
    ...
    printk(KERN_INFO "WFS: Task PID %d SWITCHED_FROM WFS class on CPU %d, %u tasks in queue\n", 
           p->pid, cpu, wfs_rq->wfs_nr_running);
...
/* Existing initialization function already handles everything */
